...
export const endOfDay = (d) => { const nd = new Date(d); nd.setHours(23,59,59,999); return nd; };
...
export function buildPeriods(start, end, mode) {
  ...
  if (mode === "daily") {
    ...
    while (cur <= end) {
      periods.push({ key: fmtYMD(cur), label: fmtYMD(cur), start: cur, end: endOfDay(cur) });
      cur = addDays(cur,1);
    }
  } else if (mode === "weekly") {
    let cur = startOfWeek(start);
    while (cur <= end) {
      const label = `Wk of ${fmtYMD(cur)}`;
      periods.push({ key: label, label, start: cur, end: endOfDay(addDays(cur,6)) });
      cur = addWeeks(cur,1);
    }
    ...
    while (cur <= end) {
      ...
      const lastDay = toDate(new Date(cur.getFullYear(), cur.getMonth()+1, 0));
      periods.push({ key: label, label, start: cur, end: endOfDay(lastDay) });
      cur = addMonths(cur,1);
    }
  } else if (mode === "yearly") {
    ...
    while (cur <= end) {
      ...
      const lastDay = toDate(new Date(cur.getFullYear(), 11, 31));
      periods.push({ key: label, label, start: cur, end: endOfDay(lastDay) });
      cur = addYears(cur, 1);
    }
  } else {
